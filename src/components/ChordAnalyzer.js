...
const ChordAnalyzer = ({ currentQuestion }) => {
    ...
    const analyzeChord = () => {
        ...
        // Console log the functions of the winning chord
        console.log('Winning chord functions:', mostStableInversion);

        return { symbol, functions: mostStableInversion };
    };

    const { symbol, functions } = analyzeChord();

    return (
        <div className="chord-analyzer">
            <h3 className="title is-3">Chord</h3>
            <p className="subtitle is-2">{symbol}</p>
        </div>
    );
};
